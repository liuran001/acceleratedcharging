...
func uninstalltemperaturecontrol() {
    var temperature = string(shell("cat /sys/class/power_supply/battery/temp"))
    var temperaturemax = string(shell("cat /data/adb/modules/acceleratedcharging/wall.ini"))
        ...
        shell("echo 3300000 > /data/adb/modules/acceleratedcharging/constant_charge_current")
        fmt.Println("达到温度墙"+ temperaturemax +" 降低充电电流:33w")
        } else {
            shell("echo 12200000 > /data/adb/modules/acceleratedcharging/constant_charge_current")
            fmt.Println("充电电流:120w")
        }
